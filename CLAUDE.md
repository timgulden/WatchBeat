...
3. **Two pickers, one architecture.** The pipeline exposes `measureWithDiagnostics` (production picker, used by the AnalyzeSamples CLI) and `measureReferenceWithDiagnostics` (Reference picker, used by the iOS app). Both share the same shape: highpass → Envelope FFT (rate candidates) → Try-all-rates with guided tick extraction → Linear regression → MeasurementResult. They differ in candidate scoring, outlier rejection, and how they handle harmonic confusion (see rules 6 and 7). Both now run `MatchedFilterRefinement` after the per-class outlier rejector to refine each tick position against an averaged tick template — the "whole 15 s of compute" lever a single-pass timegrapher doesn't have. The Reference picker is the active one for the user-facing app; the production picker is preserved for offline diagnostics and corpus testing.
...
6. **Adaptive narrow bandpass, with broadband fallback.** Tick energy lives above 4 kHz; room rumble, hum, mic self-noise, and broadband environmental noise (HVAC, voices) actively confuse the envelope FFT's rate decision. The Reference picker runs an STFT-based band-selection pass on the raw signal (`MultibandSelector`): it scores each frequency bin's rhythmicity at the standard mechanical beat rates and picks the narrow band (±500 Hz around the winning bin) with the highest peak-to-baseline ratio. If the best narrow band's score exceeds the broadband (5–22 kHz) baseline by 1.5×, the picker bandpasses there; otherwise it falls back to a 5 kHz highpass (`MeasurementPipeline.highpassCutoffHz`). The blanket "bandpass is out" rule from the original 5-kHz-highpass-only design no longer holds — the bandpass is *adaptive*, so it preserves sharp-transient content within the chosen band, and the fallback gate prevents narrowing on watches where broadband already wins. Set `WATCHBEAT_NO_MULTIBAND=1` to disable selection and run broadband-only for A/B diagnostics. The production picker still runs dual-pass (5 kHz / 8 kHz, whichever yields higher quality) for offline corpus testing.
...
9. **Audio persistence is bounded and consent-gated.** Two paths persist audio:
   (a) `#if DEBUG`-only `saveRawAudio` in `MeasurementCoordinator.swift` — writes a WAV to the app's Documents folder for corpus testing during development. Release binaries don't contain this code path.
