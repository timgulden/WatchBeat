...
9. **Release builds must never persist audio or motion data.** The App Store listing tells users "microphone audio is never saved to disk, never transmitted off your device" and the same for motion data. This is a load-bearing privacy promise — violating it is grounds for store removal and a credibility problem. Persistence-touching code that exists for development convenience (e.g. `saveRawAudio` in `MeasurementCoordinator.swift`) MUST be guarded by `#if DEBUG` at both call site AND function definition, so Release binaries don't even contain the code path. `Info.plist` does include `UIFileSharingEnabled` and `LSSupportsOpeningDocumentsInPlace` for development convenience (Tim grabs WAVs off device for corpus testing); these are harmless in Release because the `#if DEBUG` gate ensures Release builds never write anything — the user-visible folder is always empty. (Earlier attempts at per-config Info.plist via `INFOPLIST_KEY_*` build settings were unreliable; folder visibility didn't propagate to the built bundle. The unconditional approach is the reliable one.) If a future feature genuinely needs persistence (export, share-recording), update the App Store listing copy first; do not silently change behavior.
